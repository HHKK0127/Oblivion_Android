...
**Opcode to command names were recovered positionally** (2,646 candidate records of which **857 pair one source statement per instruction**, 2026-09-25) and **55 of the top 60 reach 90% purity or better**, most at 100%: `0x1000` MessageBox, `0x1002` AddItem, `0x1003` SetEssential, `0x100D` Activate, `0x100F` SetActorValue, `0x1010` ModActorValue, `0x1013` PlayGroup, `0x1016` StartCombat, `0x1017` StopCombat, `0x101C` AddSpell, `0x101D` RemoveSpell, `0x101E` Cast, `0x1021` Enable, `0x1022` Disable, `0x1025` PlaceAtMe, `0x1026` PlaySound, `0x1034` SayTo, `0x1036` StartQuest, `0x1037` StopQuest, `0x1039` SetStage, `0x1052` RemoveItem, `0x1053` ModDisposition, `0x1056` StartConversation, `0x1058` AddTopic, `0x1059` Message, `0x105A` SetAlert, `0x105E` Evp, `0x1064` PickIdle, `0x1072` Lock, `0x1073` Unlock, `0x1089` SetFactionRank, `0x108B` Kill, `0x108C` Resurrect, `0x1097` AddScriptPackage, `0x109E` MoveTo, `0x10A5` RemoveMe, `0x10A8` SetFactionReaction, `0x10B1` SetCombatStyle, `0x10BB` SetCellPublicFlag, `0x10C2` SetPCExpelled, `0x10CC` SetDestroyed, `0x10DD` SetOpenState, `0x10EC` SetGhost, `0x10F8` ModPCFame, `0x10FA` ModPCInfamy, `0x1101` SCAOnActor, `0x110D` SetQuestObject, `0x1123` PlayMagicShaderVisuals, `0x1124` PlayMagicEffectVisuals, `0x1125` StopMagicShaderVisuals, `0x1129` SAA, `0x112D` ForceWeather, `0x1137` ModPCMiscStat, `0x1142` Dispel, `0x114A` AddAchievement, `0x1151` SetIgnoreFriendlyHits, `0x115D` SetSceneIsComplex, `0x115E` Autosave, `0x1164` ShowDialogSubtitles, `0x116B` PCB. The spell-effect and magic-shader commands are written with their three-letter abbreviations more often than with the long form (`pms` 55% against 15%, `sms` 77%, `pme` 67%, `forceweather` 92% against `fw` 1%), so the abbreviation is the spelling a compiler emits. `0x001E` is `return` (30 samples, 100%) - this opcode number is unrelated to the `onreset` **block type** value `0x001E` in the `begin` table above, which lives in a different namespace. `0x1076` was not named by this probe and remains open.

Scratch: `%TEMP%\async_race\probe_grammar.py` / `probe_grammar.txt` hold the positional pairing, `probe_disasm.py` / `probe_disasm.txt` the source-and-bytecode side-by-side disassembly, `probe_block_types.py` / `probe_block_types.txt` the block type table, `probe_operand_framing.py` / `probe_operand_framing.txt` the framing coverage and its unparsed samples, and `probe_keywords.py` / `probe_op_payload.txt` the keyword census that ruled `0x0017` in as `else`; `probe_cmd_names.py` / `probe_cmd_names.txt` hold the opcode-to-command table, `probe_argc.py` / `probe_argc.txt` the argument-count measurement, `probe_strings.py` / `probe_strings.txt` and `probe_strings2.py` / `probe_strings2.txt` the string family, and `probe_payloads.py` / `probe_payloads.txt` the MoveTo and PlaceAtMe dumps.
