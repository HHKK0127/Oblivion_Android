...
**Statement operands use one framing, `[u16 n][n typed tokens]`**, which decodes **12,200 of the 14,379** payload-bearing non-structural instructions (84.85%) exactly, rising to **13,161 (91.53%)** once a first byte below `0x20` is read as a 2-byte token. The token alphabet is `r` / `s` / `f` / `l` + `u16` (3 B), `n` + `u32` (5 B), `z` + `f64` (9 B), `X` + `u16` and optionally `u16` (5 B), and the bare `u16` case above. `SetStage SQ07 100` is therefore `[2][<r3>][n 100]`, `AlvalUvaniRef.addspell DarkParalyze` is `[1][<r3>]` and `0x109E` is `[4][<r>][z][z][z]`, a three-double position call. The residue is not random: **482 + 232 + 130 of it is the string family** - `0x1000`, `0x1059`, `0x1114`, `0x111B` and `0x111C` all carry `[u16 1][u16 byteLength][text]`, as in `PlayBinkFile OblivionOutr.bik` - while `0x1004` / `0x1007` / `0x1009` use a 1-byte axis token (`X` / `Y` / `Z`) plus a trailing `u16 0`, and `0x1002` / `0x1052` / `0x1076` appear to prefix a calling reference, which would make their `n` count the parameters **after** that reference rather than the total token count. **That last point is the one remaining ambiguity in the operand model** and it matters because `0x1002` is AddItem.

Scratch: `%TEMP%\async_race\probe_grammar.py` / `probe_grammar.txt` hold the positional pairing, `probe_disasm.py` / `probe_disasm.txt` the source-and-bytecode side-by-side disassembly, `probe_block_types.py` / `probe_block_types.txt` the block type table, `probe_operand_framing.py` / `probe_operand_framing.txt` the framing coverage and its unparsed samples, and `probe_keywords.py` / `probe_op_payload.txt` the keyword census that ruled `0x0017` in as `else`.
