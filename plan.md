...
**The three small integers in those payloads are compiler metadata, not source lines** (measured over 4,094 SCPT and QUST blocks, 75,954 instructions, 2026-09-25). The source-line reading of `0x001C` was falsified by dumping one record against its SCTX: in `SECindanweScript` the single `0x001C` carries `1` while the `addItem` it precedes sits on source line 10, and both `0x0016` markers carry `1` while their `if` statements sit on lines 5 and 9. What the measurements do establish is the **byte length**, which is all a decoder needs: `0x0016` / `0x0018` hold `len == payloadLength - 4` in **12,620 of 12,620** cases, `0x0017` is **exactly 2 bytes** in **968 of 968**, and `0x001C` is **exactly 4 bytes** - never the last instruction of a block and never followed directly by another `0x001C`. `0x001C` is followed by 72 distinct opcodes, most often `0x1021` Enable (983), `0x105E` (799), `0x100D` (632), `0x1022` Disable (618), `0x109E` (612) and `0x1002` AddItem (496), so it is a general marker rather than a prefix belonging to one command. Its value spans 1..37 over 37 distinct values and the leading u16 of `0x0016` spans 1..83 over 83; both are dominated by `1` and decay smoothly, so a VM may read and discard them.
