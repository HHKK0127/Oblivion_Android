...
- **`INDX` is always 2 bytes**: a scan of the top-level `QUST` GRUP in the real ESM finds **2,516 INDX subrecords, every one size 2**, with 136 distinct values in the range 0..255 (most common 10 x245, 20 x223, 100 x214). `QuestRecordParser::parseStageEntry()` (`quest/quest_record.cpp` L321-326) guards on `size < 4` and then reads 4 bytes, so it **always returns false** on real data and would over-read if the guard passed; `parseObjectiveEntry()` (L328-333) has the same shape. No live defect follows today because `QuestRecordParser::parse()` itself has no callers, but any new stage-index reader must treat INDX as u16.
- **`setStage()` has no re-entry guard**: `quest_stage_manager.cpp` L237-268 fires `onStageTransition()` unconditionally, so a repeated `SetStage` to the stage the quest is already on would re-run that stage's scripts. Real Oblivion treats a redundant `SetStage` as a no-op except where the stage's `QSDT` flag allows repeats; that flag is not yet modelled.
