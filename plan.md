...
|------|------|------|------|
| `INDX` | u16, always 2 B | 2,516 | stage index; **125 of the 390 QUST records contain no stages at all** |
| `QSDT` | u8, always 1 B | 2,978 | values `00` x2,600 / `01` x378 - **bit 0 = run once**; 5 stages carry none |
| `CTDA` | variable | 3,784 | conditions on the enclosing block |
...
Record order is fixed: `EDID DATA FULL ICON SCRI`, then per stage `INDX` followed by 1..N blocks of `QSDT [CTDA*] [CNAM] SCHR [SCDA] [SCTX] [SCRO*]`, then that stage's `QSTA` plus its `CTDA` runs. **TES4 has no `QOBJ` / `NNAM`** - there are no Skyrim-style objectives, so the journal entry is the `CNAM` text and nothing else.
