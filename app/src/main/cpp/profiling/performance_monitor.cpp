...
void MemoryProfiler::readProcStats() {
    ...
    // Use VmPeak for more accurate peak memory (absolute peak ever reached)
    // VmHWM (High Water Mark) is also valid, we use whichever is larger
    currentMetrics.peakMemory = (vmPeak > vmHWM ? vmPeak : vmHWM) * 1024;
