...
void UIHudStatusDisplay::render() {
    if (!textRenderer || !character_) return;
...
void UIHudStatusDisplay::renderBar(float x, float y, float currentValue, float maxValue,
    ...
    if (!textRenderer || maxValue <= 0.0f) return;
    ...
    textRenderer->renderText(valueText,
...
void UIHudStatusDisplay::renderHealthBar(float x, float y) {
    ...
    float healthPercent = character_->currentHealth / std::max(1.0f, character_->maxHealth);
    glm::vec3 healthColor = getHealthColor(healthPercent);
    renderBar(x, y, character_->currentHealth, character_->maxHealth, healthColor, "Health");
}

void UIHudStatusDisplay::renderManaBar(float x, float y) {
    if (!character_) return;
    float manaPercent = character_->currentMana / std::max(1.0f, character_->maxMana);
    glm::vec3 manaColor = getManaColor(manaPercent);
    renderBar(x, y, character_->currentMana, character_->maxMana, manaColor, "Mana");
}
...
void UIHudStatusDisplay::renderLevelInfo(float x, float y) {
    if (!character_ || !textRenderer) return;
