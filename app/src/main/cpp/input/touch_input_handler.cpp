...
TouchInputHandler::TouchInputHandler()
    : lastPosition(0.0f, 0.0f), dragDelta(0.0f, 0.0f), isActive(false) {
}

void TouchInputHandler::onTouchDown(float x, float y) {
    lastPosition = glm::vec2(x, y);
    isActive = true;
    dragDelta = glm::vec2(0.0f, 0.0f);
}

void TouchInputHandler::onTouchMove(float x, float y) {
    if (isActive) {
        glm::vec2 currentPos(x, y);
        dragDelta = glm::vec2(currentPos.x - lastPosition.x, currentPos.y - lastPosition.y);
